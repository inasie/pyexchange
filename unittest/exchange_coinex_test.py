...
class ExchangeCoinExTest(unittest.TestCase):

    def get_exchange(self):
        exchange = ExchangeAPI()
        return exchange.create_exchange('CoinEx')
    ...
    def test_get_ticker(self):
        ticker = self.get_exchange().get_ticker(CurrencyPair('USDT', 'XMR'))
        self.assertIsNotNone(ticker)
        logging.info(ticker)

    def test_get_orderbook(self):
        orderbook = self.get_exchange().get_orderbook(CurrencyPair('USDT', 'XRP'))
