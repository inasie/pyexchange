...
class ExchangeBiboxTest(unittest.TestCase):

    def get_exchange(self):
        exchange = ExchangeAPI()
        return exchange.create_exchange('Bibox')
    ...
    def test_get_ticker(self):
        ticker = self.get_exchange().get_ticker(CurrencyPair('BIX', 'GTC'))
        self.assertIsNotNone(ticker)
        logging.info(ticker)

    def test_get_orderbook(self):
        orderbook = self.get_exchange().get_orderbook(CurrencyPair('BIX', 'GTC'))
